...
def comments_to_df(comments, idList):
    finalList = []
    for i in range(len(comments)):
        element = [idList[i], comments[i]]
        finalList.append(element)
        pass
    df = pd.DataFrame(finalList, columns = ['commentID','body'])
    return df
...
def lambda_handler(event, context):
    ...
    LOG.info(f"Creating dataframe with values: {comments}")
    df = comments_to_df(comments, ids)
    ...
    # Write result to S3
    write_s3(df=df, bucket="ids706", name='wilson')